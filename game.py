...
class Tg(pygame.sprite.Sprite):
    def __init__(self):
        super(Tg , self).__init__()
        self.surf = pygame.image.load("tg.png").convert()
        self.surf.set_colorkey((0, 0, 0), RLEACCEL)
        self.rect = self.surf.get_rect(
            center=(
...
ADDTG = pygame.USEREVENT + 2
pygame.time.set_timer(ADDTG, 700)
...
while runningApp:
    ...
    for event in pygame.event.get():
        ...
        elif event.type == ADDTG:
            new_tg = Tg()
            tgs.add(new_tg)
            all_sprites.add(new_tg)
        if isShowMenu:
            showMenu(event)
    ...
    if runningGame:
        if bgImg == "polyu":
            screen.blit(pygame.image.load( "polyu.jpg").convert(), (0,0))   
        else:
            s = pygame.image.load( "cityu.jpg").convert()
        ...
        if pygame.sprite.spritecollideany(player, enemies) or pygame.sprite.spritecollideany(player, tgs):
            
            try:
                pygame.sprite.spritecollideany(player, enemies).suicide()
            
            except:
                pass
            try:
                pygame.sprite.spritecollideany(player, tgs).suicide()
            except:
                pass
